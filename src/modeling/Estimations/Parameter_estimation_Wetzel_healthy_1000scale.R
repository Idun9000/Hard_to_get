...
ctr_data <- read.table('/work/DC_exam/Wetzel_data_healthy.txt', header = TRUE)
subIDs <- unique(ctr_data$subjID)
nsubs <- length(subIDs)
ntrials_max <- 150

x_raw <- ctr_data$deck
X_raw <- ctr_data$wins + ctr_data$losses # Note the sign!
...
write.csv(samples_df, "posterior_samples_healthy_Wetzel_1000scale.csv", row.names = FALSE)
...
png("traceplot_Wetzel_healthy_1000scale.png", width = 1200, height = 800)
...
# Save the workspace 
save.image(file = "estimation_Wetzel_healthy_1000scale.RData")