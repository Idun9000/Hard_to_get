...
#load control data
Ahn_heroin_data <- read.table("/work/DC_exam/Hard_to_get/in/Data/raw_data/Ahn_data/Ahn_data_heroin_filtered.txt",header=TRUE)

#----------prepare data for jags models - want trial x subject arrays for choice and gain & loss ----
# identify and count unique subject IDs
subIDs <- unique(Ahn_heroin_data$subjID)
nsubs <- length(subIDs)
ntrials_max <- 100

# all choices (x) and outcomes (X)
x_raw <- Ahn_heroin_data$deck
X_raw <- Ahn_heroin_data$gain + Ahn_heroin_data$loss #note the sign!
...
for (s in 1:nsubs) {
  ...
  ntrials_all[s] <- length(x_raw[Ahn_heroin_data$subjID==subIDs[s]])
  
  #pad trials with NA if n trials < maximum (i.e. 100)
  x_sub <- x_raw[Ahn_heroin_data$subjID==subIDs[s]] 
...
# Save as PNG
ggsave("predictive_check_Ahn_heroin.png", plot = p, width = 8, height = 6, dpi = 300)
...
prettyplot <- ggplot(pred_df, aes(x = factor(sub), y = pred_success_adjust)) +
  ...
  geom_hline(aes(yintercept = chance), color = "black", linetype = "dotted", size = 0.5) +  # Dotted line for chance
  labs(
    x = "Subject",
    y = "Predicted Choice Accuracy",
    title = "Heroin Addicts (Ahn)"
  ) + ylim(0,1)
  theme_minimal()

ggsave("predictive_check_Ahn_heroin_pretty_plot.png", plot = prettyplot, width = 8, height = 6, dpi = 300)

# Save the workspace 
save.image(file = "predictive_checks_Ahn_heroin.RData") 
